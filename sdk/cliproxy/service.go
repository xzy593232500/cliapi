...
func (s *Service) Run(ctx context.Context) error {
	...
	usage.RegisterPlugin(quota.NewUsagePlugin())
	quotaDSN := strings.TrimSpace(os.Getenv("CPA_QUOTA_POSTGRES_DSN"))
	if quotaDSN == "" {
		quotaDSN = strings.TrimSpace(os.Getenv("DATABASE_URL"))
	}
	if quotaDSN != "" {
		quotaStore, errQuota := quota.NewStore(ctx, quotaDSN, 500000)
		if errQuota != nil {
			log.Warnf("failed to initialize quota store: %v", errQuota)
		} else {
			quota.SetDefaultStore(quotaStore)
		}
	}
